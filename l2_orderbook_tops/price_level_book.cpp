...
// TODO: refactor this 
unsigned int Book::get_resting_qty(unsigned int _side,
				   unsigned int distance_from_mid)
{
    unsigned int out = 0;
...
unsigned int *Book::get_tops(unsigned int total_dollar_depth)
{
	...
	this->out[_out_len - 2] =
	    this->get_resting_qty(1, total_dollar_depth);
	this->out[_out_len - 1] =
	    this->get_resting_qty(0, total_dollar_depth);
    }
